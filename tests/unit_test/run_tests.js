...
function expect_to_be_equal(value1, value2, task = "") {
  const passed =
    "\x1b[32mTEST PASSED:\x1b[0m \x1b[36mTRUE\x1b[0m " +
    "-> " +
    (task ? `\x1b[33mTASK:\x1b[0m ${task}` : task);
  const failed =
...
if (os.platform === "win32") {
  ...
  expect_to_be_equal(
    path_1,
    ".\\example\\config.js",
    "Check if it returns an accurate path (windows path)",
  );
} else {
  ...
  expect_to_be_equal(
    path_2,
    "./example/config.js",
    "Check if it returns an accurate path (unix-like path)",
  );
}
console.log("-".repeat(30));
console.log(
...
expect_to_be_equal(
  is_includes(config_object.default, config_properties),
  true,
  "Check if config_object includes allowed properties",
);
/*
